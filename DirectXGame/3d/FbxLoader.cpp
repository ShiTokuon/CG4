...
void FbxLoader::ParseMeshVertices(Model* model, FbxMesh* fbxMesh)
{
	...
	// 頂点データの数
	const int conrtolPointsCount =
		fbxMesh->GetControlPointsCount();
	// 必要数だけ頂点データ配列を確保
	Model::VertexPosNormalUvSkin vert{};
	model->vertices.resize(conrtolPointsCount, vert);
	...
	for (int i = 0; i < conrtolPointsCount; i++) {
...
void FbxLoader::ParseSkin(Model* model, FbxMesh* fbxMesh)
{
	...
	for (int i = 0; i < clusterCount; i++) {
		...
		for (int j = 0; j < controlPointIndicesCount; j++) {
			...
			// その頂点の影響を受けるボーンリストに、ボーンとウェイトを追加
